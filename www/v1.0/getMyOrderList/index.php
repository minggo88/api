...
if ($symbol == "ALL") {
    $txns = $tradeapi->get_order_list_all($userno, 'all', $symbol, $exchange, $page, $rows, $orderid, $trading_type, $order_by, $order_method, $return_type);

} else {
    // check previos address
    $txns = $tradeapi->get_order_list($userno, 'all', $symbol, $exchange, $page, $rows, $orderid, $trading_type, $order_by, $order_method, $return_type);
    // var_dump($txns); exit;
}
