...
//     $tradeapi->gen_wallet($userno_sell, $symbol, $goods_grade);
//     $wallet_symbol = $tradeapi->get_wallet($userno_sell, $symbol, $goods_grade);
// }
...
try {
    ...
    $orders_buy = $tradeapi->get_order_by_price('B', $symbol, $exchange, $price,'',$goods_grade);
    if(!$orders_buy || count($orders_buy)<1) { // 초기값 설정.
        // 매수 주문이 없으니 주문금액 전액을 지갑에서 비용 차감. 즉, 코인(BTC) 차감.
        $tradeapi->charge_sell_price($userno_sell, $symbol, $volume, $goods_grade);
    }  else {
        // 매수 주문이 있을때는 아래에서 각각 매매하면서 해당 수량으로 BTC를 차감합니다.
        foreach($orders_buy as $order_buy) {

            if ($order_buy->goods_grade != $goods_grade) {
                continue;
            }
            ...
            //히스토리 남기기
            $tradeapi->set_history($symbol, $userno_sell, $userno_buy, $trade_price, '1');
        ...
        if( $remain_volume_sell > 0 ) {
            $tradeapi->charge_sell_price($userno_sell, $symbol, $remain_volume_sell, $goods_grade); // 남은 주문 수량 차감.
        }
    }
