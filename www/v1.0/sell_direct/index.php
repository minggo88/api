...
$max_buy_price = $tradeapi->get_max_buy_price($symbol, $exchange, $goods_grade);
...
$wallet_symbol = $tradeapi->get_wallet($userno_sell, $symbol, $goods_grade);
$wallet_symbol = $wallet_symbol ? $wallet_symbol[0] : null;
// 잔액이 있어야 하기때문에 구지 지갑을 여기서 생성하지 않습니다. 없으면 잔액 부족으로 처리되도록 합니다.
// if(!$wallet_symbol || !$wallet_symbol->userno) {// 판매자 지갑 없으면 생성. address -> userno
//     $tradeapi->create_new_trade_wallet($userno_sell, $symbol, $goods_grade);
//     $wallet_symbol = $tradeapi->db_get_row('js_exchange_wallet',  array('userno'=>$userno_sell, 'symbol'=>$symbol, 'goods_grade'=>$goods_grade));
//     if(!$wallet_exchange) {
//         $tradeapi->error('047',__($symbol.'지갑을 생성하지 못했습니다.'));
//     }
// }
// check locked
if($wallet_symbol && $wallet_symbol->locked != 'N') {
	$tradeapi->error('048', str_replace('{symbol}', $symbol, __('{symbol}지갑이 잠겨있어 매도하실 수 없습니다.')));
}
// if($wallet_symbol && $wallet_symbol->bool_sell=='0') {
// 	$tradeapi->error('048-1',__('지갑이 잠겨있어 매도하실 수 없습니다.'));
// }

$wallet_exchange = $tradeapi->get_wallet($userno_sell, $exchange);
$wallet_exchange = $wallet_exchange ? $wallet_exchange[0] : null;
if(!$wallet_exchange || !$wallet_exchange->userno) {// 구매자 지갑 없으면 생성.
    ...
    if(!$wallet_exchange) {
        $tradeapi->error('049',__($exchange.'지갑을 생성하지 못했습니다.'));
    }
}

// check locked
if($wallet_exchange->locked != 'N') {
	$tradeapi->error('050', str_replace('{symbol}', $exchange, __('{symbol}지갑이 잠겨있어 매도하실 수 없습니다.')));
}
...
try {
    ...
    $r = $tradeapi->write_sell_order($userno_sell, $address_sell, $symbol, $exchange, $price, $volume, $amount, $goods_grade);
    ...
    //     foreach($orders_buy as $order_buy) {
            ...
            $tradeapi->charge_sell_price($userno_sell, $symbol, $trade_volume, $goods_grade);
            ...
            // 구매내역 수정.(거래)
            $r = $tradeapi->trade_order($orderid_buy, $symbol, $exchange, $trade_volume, $trade_status_buy, $goods_grade);
    ...
    // 알림
    if (!$trade_status_sell) {
        $tradeapi->put_message(2, $userno_sell,"[{$currency->name} ({$goods_grade})] {$amount} ({$volume}개) KRW 판매 대기 되었습니다.");
    } else if ($trade_status_sell == "C") {
        $tradeapi->put_message(2, $userno_sell,"[{$currency->name} ({$goods_grade})] {$amount} ({$volume}개) KRW 판매 완료 되었습니다.");
    }
