...
// 가입
$sql = "insert into js_member set userid='{$exchangeapi->escape($userid)}', userpw='{$exchangeapi->escape($userpw)}', `name`='{$exchangeapi->escape($name)}', nickname='{$exchangeapi->escape($nickname)}', phone='',email='{$exchangeapi->escape($email)}',mobile='{$exchangeapi->escape($mobile)}',zipcode='',address_a='',address_b='',level_code='',regdate=UNIX_TIMESTAMP(), bool_sms=0, bool_email='{$exchangeapi->escape($bool_email)}', bool_marketing='{$exchangeapi->escape($bool_marketing)}', pin='{$exchangeapi->escape($pin)}', mobile_country_code='{$exchangeapi->escape($mobile_country_code)}' ";
...
if(__API_RUNMODE__=='live') {
    if(!$exchangeapi->query_one("select address from js_exchange_wallet where symbol='ETH' and userno='{$member->userno}' ")) {
        ...
        $exchangeapi->write_log('이더지갑 생성 끝. address: '. $address);
