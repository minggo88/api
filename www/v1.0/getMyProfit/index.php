...
// 지갑 가져오기
$wallet = $tradeapi->query_list_object("SELECT w.symbol, w.confirmed, w.address, c.name, c.symbol, c.icon_url, w.goods_grade, m.meta_val FROM js_exchange_wallet w LEFT JOIN js_trade_currency c ON w.symbol=c.symbol LEFT JOIN js_auction_goods_meta m ON m.goods_idx = w.symbol WHERE w.userno='{$tradeapi->escape($userno)}' AND c.symbol IS NOT NULL AND c.symbol NOT IN ('AAT', 'NFTN', 'USD', 'ETH') AND m.meta_key = 'meta_wp_production_date' ");

// 지갑별 상세 정보
foreach($wallet as $w) {
	$SYMBOL = strtoupper($w->symbol);
	$symbol = strtolower($w->symbol);
    $goods_grade = $w->goods_grade;
	...
	if($SYMBOL != 'KRW' && $SYMBOL != 'USD') { // 매매 종목의 잔액 ... 게산하고
		...
		$sql = "SELECT income + buy - outgoing - sell balance,  income, outgoing, sell FROM (
			...
			(SELECT IFNULL(SUM(volume),0) FROM js_trade_{$tradeapi->escape($symbol)}{$tradeapi->escape($exchange)}_txn t LEFT JOIN js_trade_{$tradeapi->escape($symbol)}{$tradeapi->escape($exchange)}_ordertxn ot ON t.txnid=ot.txnid WHERE ot.userno='{$tradeapi->escape($userno)}' AND t.goods_grade='{$tradeapi->escape($goods_grade)}' AND t.orderid_sell=ot.orderid AND time_traded<'{$tradeapi->escape($sdate)} 00:00:00' ) sell,
			(SELECT IFNULL(SUM(volume),0) FROM js_trade_{$tradeapi->escape($symbol)}{$tradeapi->escape($exchange)}_txn t LEFT JOIN js_trade_{$tradeapi->escape($symbol)}{$tradeapi->escape($exchange)}_ordertxn ot ON t.txnid=ot.txnid WHERE ot.userno='{$tradeapi->escape($userno)}' AND t.goods_grade='{$tradeapi->escape($goods_grade)}' AND t.orderid_buy=ot.orderid AND time_traded<'{$tradeapi->escape($sdate)} 00:00:00' ) buy
			) t";
		...
		// 기초평가금액
		$basic_evaluation_amount = real_number( $s_balance * $s_price , 6);
		...
		$sql = "SELECT income + buy - outgoing - sell balance,  income, outgoing, sell FROM (
			...
			(SELECT IFNULL(SUM(volume),0) FROM js_trade_{$tradeapi->escape($symbol)}{$tradeapi->escape($exchange)}_txn t LEFT JOIN js_trade_{$tradeapi->escape($symbol)}{$tradeapi->escape($exchange)}_ordertxn ot ON t.txnid=ot.txnid WHERE ot.userno='{$tradeapi->escape($userno)}' AND t.goods_grade='{$tradeapi->escape($goods_grade)}' AND t.orderid_sell=ot.orderid AND time_traded<'{$tradeapi->escape($next_date)} 00:00:00' ) sell,
			(SELECT IFNULL(SUM(volume),0) FROM js_trade_{$tradeapi->escape($symbol)}{$tradeapi->escape($exchange)}_txn t LEFT JOIN js_trade_{$tradeapi->escape($symbol)}{$tradeapi->escape($exchange)}_ordertxn ot ON t.txnid=ot.txnid WHERE ot.userno='{$tradeapi->escape($userno)}' AND t.goods_grade='{$tradeapi->escape($goods_grade)}' AND t.orderid_buy=ot.orderid AND time_traded<'{$tradeapi->escape($next_date)} 00:00:00' ) buy
			) t";
		$e_balance = $tradeapi->query_one($sql);
		// 기말가격
		$e_price = $tradeapi->query_one("SELECT close FROM `js_trade_{$tradeapi->escape($symbol)}{$tradeapi->escape($exchange)}_chart` WHERE `date`='{$tradeapi->escape($edate)} 00:00:00' AND goods_grade='{$tradeapi->escape($goods_grade)}' AND term='1d'");
		// 기말평가금액
		$final_evaluation_amount = real_number( $e_balance * $e_price , 6 );
		...
		$sql = "SELECT IFNULL(SUM(t.amount* c.close),0) amount FROM js_exchange_wallet_txn t LEFT JOIN js_trade_{$tradeapi->escape($symbol)}{$tradeapi->escape($exchange)}_chart c ON c.date=DATE(t.txndate) AND c.term='1d' WHERE t.userno='{$tradeapi->escape($userno)}' AND c.goods_grade='{$tradeapi->escape($goods_grade)}' AND t.txn_type<>'B' AND t.direction='I' AND `status` <> 'C' AND t.symbol='{$tradeapi->escape($SYMBOL)}' AND '{$tradeapi->escape($sdate)} 00:00:00'<=txndate AND txndate<'{$tradeapi->escape($next_date)} 00:00:00' "; // 기간 입금액 SUM(입금수량 * 가격)
		...
		$sql = "SELECT IFNULL(SUM(t.amount* c.close),0) amount FROM js_exchange_wallet_txn t LEFT JOIN js_trade_{$tradeapi->escape($symbol)}{$tradeapi->escape($exchange)}_chart c ON c.date=DATE(t.txndate) AND c.term='1d' WHERE t.userno='{$tradeapi->escape($userno)}' AND c.goods_grade='{$tradeapi->escape($goods_grade)}' AND t.txn_type<>'B' AND t.direction='O' AND `status` <> 'C' AND t.symbol='{$tradeapi->escape($SYMBOL)}' AND '{$tradeapi->escape($sdate)} 00:00:00'<=txndate AND txndate<'{$tradeapi->escape($next_date)} 00:00:00' "; // 기간 입금액 SUM(입금수량 * 가격)
		...
		$coin = (object) array(
			'symbol'=>$w->symbol,
			'icon_url'=>$w->icon_url,
            'production_date'=>$w->meta_val,
			'name'=>__($w->name).' / '.$w->goods_grade ,
			// 'sell_amount'=>$sell_amount,
			'basic_balance'=> real_number_format($s_balance),
			'basic_price'=>$s_price,
			'basic_evaluation_amount'=>$basic_evaluation_amount,
			'final_balance'=> real_number_format($e_balance),
			'final_price'=>$e_price,
		...
		$r->detail[$w->symbol.'/'.$w->goods_grade] = $coin;

		// 전체 데이터
		$r->total->investment_amount += real_number_format($coin->investment_amount);         // 투자금액 = ∑ 자산별 (기초평가금액 + ∑입금평가금액 - ∑출금평가금액)
		...
		$sql = "SELECT income - outgoing + sell - buy balance,  income, outgoing, sell FROM (
			SELECT
			(SELECT IFNULL(SUM(amount),0) FROM js_exchange_wallet_txn WHERE symbol='{$tradeapi->escape($SYMBOL)}' AND userno='{$tradeapi->escape($userno)}' AND txndate<'{$tradeapi->escape($sdate)} 00:00:00' AND direction='I' AND txn_type<>'B' AND `status` = 'D') income,
			(SELECT IFNULL(SUM(amount),0) FROM js_exchange_wallet_txn WHERE symbol='{$tradeapi->escape($SYMBOL)}' AND userno='{$tradeapi->escape($userno)}' AND txndate<'{$tradeapi->escape($sdate)} 00:00:00' AND direction='O' AND txn_type<>'B' AND `status` <> 'C') outgoing,";
		// sell
		$sql.= "( 0 ";
		...
		$sql.= ") buy ";
		$sql.= ") t";
		$basic_evaluation_amount = real_number( $tradeapi->query_one($sql), 6);

		// 기말잔고
		$sql = "SELECT income - outgoing + sell - buy balance,  income, outgoing, sell FROM (
			SELECT
			(SELECT IFNULL(SUM(amount),0) FROM js_exchange_wallet_txn WHERE symbol='{$tradeapi->escape($SYMBOL)}' AND userno='{$tradeapi->escape($userno)}' AND txndate<'{$tradeapi->escape($next_date)} 00:00:00' AND direction='I' AND txn_type<>'B' AND `status` = 'D') income,
			(SELECT IFNULL(SUM(amount),0) FROM js_exchange_wallet_txn WHERE symbol='{$tradeapi->escape($SYMBOL)}' AND userno='{$tradeapi->escape($userno)}' AND txndate<'{$tradeapi->escape($next_date)} 00:00:00' AND direction='O' AND txn_type<>'B' AND `status` <> 'C') outgoing,";
		// sell 
		$sql.= "( 0 ";
		foreach($currency as $c) {
		...
		$sql.= ") t";
		// var_dump($sql);
		$final_evaluation_amount = real_number( $tradeapi->query_one($sql) , 6);
		...
		$coin = (object) array(
			...
			'name'=>__($w->name),
			// 'sell_amount'=>$sell_amount,
			'basic_balance'=> real_number_format($basic_evaluation_amount),
			'basic_evaluation_amount'=>$basic_evaluation_amount, // 기초평가금액
			'final_balance'=> $edate==date('Y-m-d') ? $w->confirmed : real_number_format($final_evaluation_amount),
		...
		$r->total->deposit_evaluation_amount += $coin->deposit_evaluation_amount;   // 입금평가금액 = 기간 내 입금 USD + 입금 가상자산 평가금액 (해당시간 시세기준)
		$r->total->withdraw_evaluation_amount += $coin->withdraw_evaluation_amount;   // 출금평가금액 = 기간 내 출금 USD + 출금 가상자산 평가금액 (해당시간 시세기준)
		$r->total->available_withdraw_amount = real_number_format( $w->confirmed ); // 출금가능원화은 현제 잔액으로 표시.  $coin->final_evaluation_amount; // 출금가능금액 = USD 잔액
		$r->total->available_order_amount = real_number_format( $w->confirmed ); // 주문가능원화은 현제 잔액으로 표시.  $coin->final_evaluation_amount;    // 주문가능금액 = USD 잔액
	}

}

$r->total->investment_income = real_number_format( $r->total->investment_income ) ;
