...
$goods_idx      = checkEmpty($_REQUEST['goods_idx'], 'goods_idx');          //경매상품번호
$start_price    = checkEmpty($_REQUEST['start_price'], 'start_price');      //판매시작금액
...
$price_symbol   = setDefault($_REQUEST['price_symbol'], 'ETH');             //거래화폐(가격)심볼
...
if(!$goods_info || !$goods_info->goods_idx){
...
if($auction_idx){
    ...
    if(isset($_REQUEST['auction_title']) && $auction_title!=$goods_info->auction_title ) { $sql.= ",auction_title='{$tradeapi->escape($auction_title)}' "; }
    if(isset($_REQUEST['start_date']) && $start_date!=$goods_info->start_date ) { $sql.= ",start_date='{$tradeapi->escape($start_date)}' "; }
    if(isset($_REQUEST['end_date']) && $end_date!=$goods_info->end_date ) { $sql.= ",end_date='{$tradeapi->escape($end_date)}' "; }
    if(isset($_REQUEST['start_price']) && $start_price!=$goods_info->start_price ) { $sql.= ",start_price='{$tradeapi->escape($start_price)}' "; }
    // if(isset($_REQUEST['sell_price']) && $sell_price!=$goods_info->sell_price ) { $sql.= ",sell_price='{$tradeapi->escape($sell_price)}' "; }
    if(isset($_REQUEST['wish_price']) && $wish_price!=$goods_info->wish_price ) { $sql.= ",wish_price='{$tradeapi->escape($wish_price)}' "; }
    if(isset($_REQUEST['unit_price']) && $unit_price!=$goods_info->unit_price ) { $sql.= ",unit_price='{$tradeapi->escape($unit_price)}' "; }
    if(isset($_REQUEST['buy_now']) && $buy_now!=$goods_info->buy_now ) { $sql.= ",buy_now='{$tradeapi->escape($buy_now)}' "; }
    if(isset($_REQUEST['sell_volume']) && $sell_volume!=$goods_info->sell_volume ) { $sql.= ",sell_volume='{$tradeapi->escape($sell_volume)}' "; }
	...
	$sql = "INSERT INTO js_auction_list SET auction_idx='{$auction_idx}', goods_idx='{$tradeapi->escape($goods_idx)}', auction_title='{$tradeapi->escape($auction_title)}', start_date='{$tradeapi->escape($start_date)}', end_date='{$tradeapi->escape($end_date)}', start_price='{$tradeapi->escape($start_price)}', sell_price='0', sell_volume='{$tradeapi->escape($sell_volume)}', wish_price='{$tradeapi->escape($wish_price)}', unit_price='{$tradeapi->escape($unit_price)}', creator_userno='{$tradeapi->escape($userno)}', buy_now='{$tradeapi->escape($buy_now)}',  finish='N', reg_date=NOW() ";
    $result = $tradeapi->query($sql);
    if($result) {
        $tradeapi->query("UPDATE js_auction_goods SET price='{$tradeapi->escape($start_price)}' WHERE idx='{$tradeapi->escape($goods_idx)}' ");
    }
