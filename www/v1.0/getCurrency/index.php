...
// 전체 조회시.
switch($symbol) {
    case 'ALL': $symbol = ''; break;
    case 'WISH': 
        $symbol = $tradeapi->query_list_one("SELECT target_idx FROM js_subscribe WHERE subscriber_userno='{$tradeapi->escape($userno)}' AND target_type='trade' AND `like`='Y' ");
        if(!$symbol) {
            $symbol = array('없는심볼');
        }
        break;
    case 'HOLD':
        $symbol = $tradeapi->query_list_one("SELECT symbol FROM js_exchange_wallet WHERE userno='{$tradeapi->escape($userno)}' AND symbol <> 'KRW' "); 
        if(!$symbol) {
            $symbol = array('없는심볼');
        }
        break;
