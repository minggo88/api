...
// 은행정보
$bank_info = array('USD', 'KRW');
foreach($row as $bank_info) {
    $bank_info[$row] = $tradeapi->query_fetch_object("select bank_name, account_no, account_user, concat(bank_name,' / ', account_no,' / ', account_user) bank_full_info  from js_config_account  WHERE coin in '{$row}' ");
}
...
for($i=0; $i<count($wallets); $i++) {
    ...
    if($wallet->symbol=='USD' || $wallet->symbol=='KRW') {
        $wallet->address = $bank_info[$wallet->symbol]->bank_full_info; // 한국어 은행.
