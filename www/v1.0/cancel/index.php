...
try {
    ...
    if($order->trading_type == 'buy') { // 구매시 선취차감한 KRW를 되돌려줍니다.
        ...
        $tradeapi->add_wallet($userno, $exchange, $_refund, $goods_grade);
    } else { // 판매시 선취차감한 코인을 되돌려줍니다.
        $tradeapi->add_wallet($userno, $symbol, $order->volume_remain, $goods_grade);
    }
    ...
    $tradeapi->set_quote_data($symbol, $exchange, $order->price, $goods_grade); // 주문 가격으로 전달 받은 가격에 대해 호가를 갱신합니다.
