...
for ($day = 1; $day <= 100; $day++) {
    ...
    $now = date('Y-m-d', time()-60*60*24*$day);
    // $tradeapi->write_log('$day:'.$day);
    $tradeapi->write_log('$now:'.$now);
    // var_dump($now); ob_flush();
...
// var_dump($sql);exit;
    if ($sql) {
        // 전체거래종목 평가금액
        // var_dump("select SUM(amount) from ( " . implode(' UNION ALL ', $sql) . " ) t "); exit;
